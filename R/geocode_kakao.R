...
cat(" \033[1;32m my_kakao_rest <- 'YOUR_KAKAO_REST_KEY'  \033[0m ", '\n')
cat(" \033[1;32m   \033[0m ", '\n')
...
cat(" \033[1;32m     newdf$long_x[i] <- longlat[1]       \033[0m ", '\n')
cat(" \033[1;32m     newdf$lat_y[i] <- longlat[2]  \033[0m ", '\n')
...
long_x <- tmp$documents.x[1]  # 경도(longitude)
lat_y  <- tmp$documents.y[1]  # 위도(latitude)
tmp2 <- c(long_x, lat_y)
return(tmp2)
