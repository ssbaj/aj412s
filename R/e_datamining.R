...
if(explaining==0) {
cat("## CSV파일과 Excel파일 로딩하기 ------- ", '\n')
cat("  library(aj412s) ", '\n')
cat("  df<-opencsv('TESTdata1.csv', header=T, skip=1) ", '\n')
cat("  df<-openxlsx('TESTdata2.xlsx', header=T, skip=1) ", '\n')
cat("   ", '\n')
cat("## 화면 스크롤 갯수 확대 & 미싱데이터 채우기 ------- ", '\n')
cat("  print(tmp, n=99999)  ", '\n')
cat("  options(max.print=10000)  ", '\n')
cat("  options(dplyr.print_max=10000)  ", '\n')
cat("  zoo::na.locf(df)  ", '\n')
cat("   ", '\n')
cat("## NA미싱데이터 삭제하기 ------- ", '\n')
