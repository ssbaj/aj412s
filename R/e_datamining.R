...
cat("  df<-df[complete.cases(df$age), ]  ", '\n')
cat("  ", '\n')
...
cat("  index <- c(2,4,6)  ", '\n')
cat("  New_df <- lapply(df[, index], as.character)  ", '\n')
cat("  ", '\n')
...
cat("  library(aj412s)  ", '\n')
cat("  df<-BasicData  ", '\n')
cat("  df<-df[complete.cases(df$conv), ]  ", '\n')
cat("  df<-mkdum22(df, conv)  ", '\n')
cat("  ", '\n')
...
cat("  table(df$성별, df$브랜드)   ", '\n')
cat("  barplot(table(df$성별, df$브랜드), beside=T, horiz=F, ylab='빈도수')   ", '\n')
cat("  ", '\n')
...
cat("  ", '\n')
cat("  df <- data.frame(category = c('A', 'B', 'A', 'C', 'B'))  ", '\n')
cat("  df %>% mutate(category = recode(category, 'A' = 'Apple', 'B' = 'Banana', 'C' = 'Cherry'))  ", '\n')
cat(" ", '\n')
} }
