...
cat("\033[1;33m## 패키지 설치 ------- \033[0m ", '\n')
cat("  devtools::install_github('ssbaj/aj412s2') ", '\n')
cat("  devtools::install_github('ssbaj/dfclass')", '\n')
cat("  ", '\n')
cat("\033[1;33m## Working folder 위치 확인/지정 ------- \033[0m ", '\n')
cat("  getwd() # 현재 working folder 위치 ", '\n')
cat("  setwd('c:/dataPy') # working folder 지정 ", '\n')
cat("  list.files() # working folder의 파일 리스트 ", '\n')
cat("  ", '\n')
cat("\033[1;33m## 화면 스크롤의 저장 -------  \033[0m ", '\n')
cat("  sink('결과저장.txt')   ", '\n')
cat("    ……   ", '\n')
cat("  sink()   ", '\n')
cat("  ", '\n')
...
cat("\033[1;34m**************************************************************** \033[0m ", '\n')
cat("\033[1;33m## Sequence data 만들기 -------\033[0m ", '\n')
cat("#\033[1;32m 처음 숫자는 100, 마지막 숫자는 200, 총 자료수는 20개인 자료 \033[0m", '\n')
cat("  seq(100, 200, (200-100)/(20-1))  # 100부터 시작해서 200까지, 총 자료수는 20개 ", '\n')
cat("  ", '\n')
cat("\033[1;33m## Repeat data 만들기 -------\033[0m ", '\n')
cat("#\033[1;32m seq(1:5)을 3회 반복한 자료  \033[0m", '\n')
cat("  rep(seq(1:5), 3) ", '\n')
cat("  ", '\n')
cat("\033[1;33m## data.frame 만들기 -------\033[0m ", '\n')
cat("  set.seed(123)  # 재현 가능하게 ", '\n')
cat("  df <- data.frame( ", '\n')
cat("    id    = 1:5, ", '\n')
cat("    brand = sample(c('KIA', 'HD', 'Toyota'), size = 5, replace = TRUE), ", '\n')
cat("    conv  = sample(1:5, size = 5, replace = TRUE) ", '\n')
cat("    ) ", '\n')
...
cat("\033[1;33m## 로딩한 csv파일은 data.frame -------\033[0m ", '\n')
cat("  df<-read.csv('파일명.csv') # 또는  ", '\n')
cat("  ", '\n')
cat("  library(aj412s2) ", '\n')
cat("  df<-opencsv(파일명.csv, header=T, skip=number of skipped lines) ", '\n')
cat("  ", '\n')
cat("\033[1;33m## 로딩한 xlsx/xls파일은 data.frame -------\033[0m ", '\n')
cat("  library(readxl) ", '\n')
cat("  df <- read_excel('파일명.xlsx', skip=number of skipped lines,  ", '\n')
cat("        col_names = header, sheet=sheet number) ", '\n')
cat("  ", '\n')
cat("  library(aj412s2) ", '\n')
cat("  df <- openxlsx(파일명.xlsx, header=T, skip=number, sheet=sheet number)", '\n')
cat("  ", '\n')
cat("\033[1;33m## Matrix data 만들기 -------\033[0m ", '\n')
cat("  x<-matrix(1:9, nrow=3, byrow=T) ", '\n')
cat("  x[1,3]", '\n')
cat("  ", '\n')
cat("\033[1;33m## List data 만들기 -------\033[0m ", '\n')
cat("  x<-rep(1:3, 3)  ", '\n')
cat("  df<-data.frame(  ", '\n')
cat("     id=c(1,2,3,4),  ", '\n')
...
cat("\033[1;33m## 변수에 설명 추가 ------- \033[0m ", '\n')
cat("  library(aj412s2) ", '\n')
cat("  library(Hmisc) ", '\n')
cat("   ", '\n')
cat("  df<-BasicData ", '\n')
...
cat("   ", '\n')
cat("  df2<-df[ ,-c(1,3)]  # 첫 번째, 세 번째 변수를 삭제", '\n')
cat("  df2<-df[df$gender=='남', ]  ", '\n')
cat("  df2<-df[df$gender=='남', c(2, 3)]  ", '\n')
cat("  df2<-df[df$id>=3, c(2, 3)]  ", '\n')
cat("   ", '\n')
cat("\033[1;33m## table사용법 --------------  \033[0m ", '\n')
cat("\033[1;33m  ## table을 사용해 막대그래프 그리기 \033[0m ", '\n')
cat("  table(df$gender, df$day)   ", '\n')
cat("  barplot(\033[1;34mtable\033[0m(df$gender, df$day), beside=T, horiz=F, ylab='빈도수')   ", '\n')
cat("  ", '\n')
...
cat("  df<-icecream ", '\n')
cat("  p <- \033[1;33mggplot\033[0m(df, aes(온도, 판매량) ) + ", '\n')
cat("       \033[1;33mgeom_point\033[0m(color='purple', size=4, alpha=0.7) + ", '\n')
cat("       \033[1;33mgeom_smooth\033[0m(method = lm', color='blue', lwd=0.8, se=FALSE) +  ", '\n')
cat("       mytheme  # geom_smooth의 method='loess'  ", '\n')
cat("  print(p) ", '\n')

invisible(readline(prompt="Press [enter] to continue"))

cat("\033[1;34m**************************************************************** \033[0m ", '\n')
cat("\033[1;34m## data.frame 변수의 조작 ------- \033[0m", '\n')
...
cat("\033[1;31m  # 또는... \033[0m ", '\n')
cat("  df<-comp22(df) ", '\n')
cat("  df<-comp22(df, conv, satprice) ", '\n')
cat("  ", '\n')
cat("\033[1;33m## 변수를 선택해서 새 데이터셋으로 만들기 ---- \033[0m ", '\n')
cat("  df2 <- df[ , c(2, 3, 4:6)] ", '\n') # 변수 순서를 지정해 선택함

cat("  df2 <- df %>% select(brand, year, month:age) ", '\n') # 변수 순서를 지정해 선택함
cat("  df2 <- sel22(df, id, dept, brand, age ) ", '\n')
cat("\033[1;31m  # 또는... \033[0m ", '\n')
cat("  index <- c(1, 2, 3, 9)  ", '\n')
cat("  df2 <- df[ , index]  ", '\n')
cat("  ", '\n')
...
cat("  df2 <- lapply(df[, index], as.numeric)  ", '\n')
cat("  ", '\n')
...
cat("  for(i in 1:10) { cat(i,'th number is Right!', '\\n') }  # R명령문 ", '\n')
cat("  ", '\n')
cat("\033[1;34m  for i in range(1, 11):  # Python명령문 \033[0m ", '\n')
cat("\033[1;34m       print(i, 'hellow', 'world', '\\n') \033[0m ", '\n')
cat("  ", '\n')
cat("\033[1;33m## rowSums 및 rowMeans --------- \033[0m ", '\n')
cat("  ", '\n')
cat("  library(aj412s2) ; library(dfclass)", '\n')
cat("  Adata<-df0 ", '\n')
...
cat("\033[1;33m## BasicData는 aj412s2 패키지 내장 데이터 \033[0m ", '\n')
cat("  df <- BasicData %>% \033[1;36mmutate\033[0m(gm.satprice = satprice-mean(satprice, na.rm=T),   ", '\n')
cat("                             새변수 = paste(brand, year, sep = '_' ),  ", '\n')
cat("                             gm.conv = conv-mean(conv, na.rm=T))  ", '\n')
cat("  ", '\n')
cat("\033[1;32m## recode 명령문 사용 시 변수를 '%>%'으로 연결 금지 \033[0m ", '\n')
cat("  df$brand_se <- \033[1;32mrecode\033[0m(df$brand, 'old값1' = 'new값1', 'old값2' = 'new값2') ", '\n')
cat("  ", '\n')
...
cat("  ", '\n')
cat("  df %>% \033[1;36mfilter\033[0m(변수E==2015 & 변수M!=2015 | (변수F=='C' | 변수F=='G') )  ", '\n')
...
cat("\033[1;32m  # 또는,\033[0m colnames(df)[1]<-c('새변수명')  ", '\n')
cat("  ", '\n')
cat("  df <- \033[1;36mmerge\033[0m(data1, data2, by='id')  ", '\n')
cat("                 %>% \033[1;36mmerge\033[0m(data3, by='id')  ", '\n')
cat("     ", '\n')
cat("  df <- \033[1;36mmerge\033[0m(data1, \033[1;36mselect\033[0m(data2, -c(v3) ) , by='id')  ", '\n')
...
cat("  df<-BasicData  ", '\n')
cat("    ", '\n')
cat("\033[1;32m  unique(df$conv)  \033[0m ", '\n')
cat("  # conv의 변수값: 3 NA  2  1  4  5  ", '\n')
cat("  ", '\n')
...
cat("  library(writexl)  ", '\n')
cat("  write_xlsx(df, '파일명.xlsx')  ", '\n')
cat("\033[1;31m  # 또는... \033[0m ", '\n')
cat("  mkxlsx(df, 파일명.xlsx) ", '\n')
cat("  ", '\n')
