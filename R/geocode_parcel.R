geocode_parcel<-function(vworld_apikey, addr){
  ...
  if (base::missing(vworld_apikey)) {
    ...
    cat(" \033[1;34m df$addr <- paste0(df$시군구, ' ', df$번지) \033[0m ", '\n')
    cat(" \033[1;34m  \033[0m ",   '\n')
    ...
    cat(" \033[1;34m     도로명 = first(도로명),   \033[0m ", '\n')
    ...
    cat(" \033[1;34m my_vworld <- 'YOUR_VWORLD_API_KEY'   \033[0m ", '\n')
    cat(" \033[1;34m    \033[0m ", '\n')
    ...
    cat(" \033[1;34m       geocode_parcel(my_vworld, addr)   \033[0m ", '\n')
  ...
  url_encode_plus <- function(string) {
    ...
    encoded_string <- stringr::str_replace_all(encoded_string, "%20", "+") # 공백을 '+'로 변경
...
## -------------------------------------------------------------
  ...
  if (httr::status_code(response) == 200) {
    ...
    # JSON 데이터 파싱
    jsonData <- jsonlite::fromJSON(response_body)
    ...
    cat(addr, ' data-Error: ', 'error code:', httr::status_code(response), '\n')
  }
