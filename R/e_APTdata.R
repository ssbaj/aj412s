...
cat("# no_data를 정확히 모르면 어림셈으로 100으로 지정 후 일단 명령문을 실행해 본다 ", '\n')
cat(" ", '\n')
...
cat("---- 생략 ---- ", '\n')
cat("if(counterxx==99) {A99<-Temp } ", '\n')
cat("if(counterxx==100) {A100<-Temp } ", '\n')
cat("#-----------------------------------------------------------  ", '\n')
cat("", '\n')
cat("library(readxl) ", '\n')
cat("library(dplyr) ", '\n')
...
cat(" ", '\n')
cat("# for문에서 생성되는 Temp데이터셋을 순차적으로 다른 이름으로 저장하는 명령문이다 ", '\n')
cat("# 줄로 표시된 부분을 지우고 아래 for/if문으로 만든 명령문으로 대체한 후 명령문을 실행한다 ", '\n')
cat("#-----------------------------------------------------------  ", '\n')
cat("# 데이터셋명 교체.. 아래 for문을 콘솔에 복사해 실행시키면 명령문이 생성된다. ", '\n')
cat("# no_data를 정확히 모르면 어림셈으로 100으로 지정 후 일단 명령문을 실행해 본다 ", '\n')
cat("# 생성되는 Temp 데이터셋을 이름을 다르게 해서 저장하는 방법으로  ", '\n')
cat(" ", '\n')
cat("for(i in 1:no_data){ ", '\n')
cat("print( paste('if(counterxx==', i, ') {A', i, '<-Temp }', sep='') )  } ", '\n')
cat(" ", '\n')
