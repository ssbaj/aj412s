reshaping <- function(df, commoncol, repeating){
   ...
   if (base::missing(df)) {
		...
		cat("   *NOTE: df2<-reshaping(df, 공통컬럼은 1~k까지, (k+1)~n컬럼을 끊어서 m개씩 묶음) ", '\n')
		cat("   *NOTE: df2<-reshaping(df, 공통반복컬럼, 묶음수) ", '\n')
		return(cat("      ") ) }
  ...
  totalcol  <- ncol(df)
  blockcol  <- totalcol - commoncol   # 반복될 열의 총 개수
  repeatcol <- repeating              # 한 묶음당 열 수 (사용자가 직접 입력)
  n_repeats <- blockcol / repeatcol   # 실제 반복 횟수 (자동 계산)
